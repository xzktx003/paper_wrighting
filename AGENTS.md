...
- `HOST`、`PORT`、`WEB_HOST`、`WEB_PORT`、`WEB_BACKEND_HOST`、`WEB_BACKEND_PORT`、`WEB_HTTPS`、`FILE_BROWSER_DEFAULT_LOCAL_PATH`、`VSCODE_WEB_*` 等“用户会选择/机器相关”的配置，**严禁硬编码到源码**。
...
- 如果新增功能改变了产品概览、架构边界或使用方式，也必须同步更新 `README.md`。
...
9. 每次新增实验或新 idea 必须同步记录
- 每次新增实验（无论成功或失败），必须在 `docs/ALL_EXPERIMENTS_COMPLETE.md` 中追加记录，写明实验目的、方法原理、关键参数、结果数据和结论。
- 每次产生新的研究 idea 或优化方向，必须记录到 `docs/ideas_log.md`，写明 idea 描述、动机、预期效果、当前状态（待验证/进行中/已验证/已否决）。
- 实验结果 JSON 文件统一存放在 `experiments/results/` 目录下。
- 失败的实验同样重要，必须记录失败原因以避免重复踩坑。
