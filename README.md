...
### 在看板内打开本地 VS Code Web

- 支持为本地终端会话直接打开内嵌 `code-server` / VS Code Web 编辑器。
- 默认复用当前用户 `~/.vscode-server/extensions` 下已安装的扩展，避免为看板环境重复安装常用扩展。
- 如果需要切换共享扩展目录，可以通过 `VSCODE_WEB_EXTENSIONS_DIR` 显式覆盖默认路径。
