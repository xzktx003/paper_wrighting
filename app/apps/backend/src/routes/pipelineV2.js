...
export function registerPipelineV2Routes(fastify) {
  ...
  fastify.post('/api/v2/pipeline/start', async (request) => {
    const { preset, projectPath, chapterScope, customStages } = request.body;
    ...
    const pipeline = await createPipelineV2(definition, projectPath, resolvedPath, { chapterScope });
