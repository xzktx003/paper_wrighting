...
- 顶栏按“品牌状态 / 主操作 / 当前会话工具 / 显示设置 / 工具菜单 / 窗口控制”分组：常驻新建会话、扫描菜单、文件、VS Code、显示菜单、工具菜单、全屏和折叠入口；扫描 tmux 与扫描会话收纳到“扫描”，操作提示、资源诊断、手机端入口和 VS Code 缓存维护收纳到“工具”。
- 菜单栏折叠状态持久化到本地存储。
- 顶栏“显示”菜单提供终端预览模式切换，可在默认轻量预览和旧版完整小终端预览之间切换。
