...
- 顶栏分组展示：左侧是 `Coding Kanban` 和会话数量徽标，中间保留新建会话、扫描、文件、VS Code 等高频入口，右侧提供“显示”“工具”、全屏和折叠入口。
- `扫描` 菜单收纳扫描 tmux 和扫描会话；`显示` 菜单收纳终端预览模式；`工具` 菜单收纳操作提示、资源诊断、手机端入口和 VS Code iframe 缓存维护。
- 顶栏可折叠；折叠状态保存在 `localStorage` 的 `agent-console-layout`。
- `显示` 菜单提供终端预览模式按钮：`轻量预览：开` 为默认省资源模式，`完整预览` 恢复旧版小终端模式。
- `工具` 菜单提供 `资源诊断` 面板，打开时每秒刷新浏览器侧资源指标：xterm/终端视图数量、终端 WebSocket 数、会话快照吞吐、终端实时流吞吐、VS Code iframe 当前/隐藏数量、主线程长任务、VS Code 代理 HTTP/WS 吞吐和 Chromium JS heap；面板会给出当前压力源判读，用于区分完整预览、多终端 WebSocket、快照频率、活跃终端输出、隐藏 VS Code iframe、code-server 代理流量和真实 retained-object 泄漏。
- `工具` 菜单中的“操作提示”弹出以下提示：
  - 双击卡片放大
  - Esc 返回宫格
...
- 快捷键条发送真实终端控制字符，支持中断、Esc、Tab、Enter、EOF、方向键、清屏、行首和行尾；用户可点击“说明”查看每个快捷键的作用。
- 多行输入框通过普通 `<textarea>` 承载手机输入法，支持“发送”“粘贴”“粘贴执行”，避免依赖 xterm helper textarea 直接唤起软键盘。
