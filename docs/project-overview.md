...
- code-server 面板上的 `VS Code / code-server / 复用` 是状态标签：
  - `VS Code`：当前嵌入编辑器。
  - `code-server`：服务提供方。
  - `复用`：复用已启动的 server。
...
- VS Code Web 左上角工具组里，`VS Code / code-server / 复用` 是状态标签。
